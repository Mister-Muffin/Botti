...
export async function run(client, interaction, args) {
    ...
    client.channels.fetch(interaction.channel_id).then(async (channel) => {
        ...
        const member = await (await client.guilds.fetch(interaction.guild_id)).members.fetch(interaction.member.user.id); // ?
        if (!member.hasPermission("MANAGE_MESSAGES")) {
            client.api.interactions(interaction.id, interaction.token).callback.post({
                data: {
                    ...
                    data: {
                        content:
                            `:x: Sorry ${interaction.member.nick}, du hast nicht die nötigen Rechte!\nBitte einen Admin um Hilfe!`,
                    },
                },
            });
            return;
